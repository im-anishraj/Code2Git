...
const statusCode = (res, status, name) => {
  switch (status) {
    ...
    default:
      ...
      /* Set Repo Hook */
      const hookKey = $('#platform').val() === 'gfg' ? 'gfg_hook' : 'code2git_hook';
      chrome.storage.local.set({ [hookKey]: res.full_name }, () => {
...
const linkStatusCode = (status, name) => {
  ...
  switch (status) {
    case 301:
      $('#success').hide();
      $('#error').html(
        `Error linking <a target="blank" href="${`https://github.com/${name}`}">${name}</a> to Code2Git. <br> This repository has been moved permenantly. Try creating a new one.`,
      );
      $('#error').show();
      break;
    ...
    case 403:
      $('#success').hide();
      $('#error').html(
        `Error linking <a target="blank" href="${`https://github.com/${name}`}">${name}</a> to Code2Git. <br> Forbidden action. Please make sure you have the right access to this repository.`,
      );
      $('#error').show();
      break;
    ...
    case 404:
      $('#success').hide();
      $('#error').html(
        `Error linking <a target="blank" href="${`https://github.com/${name}`}">${name}</a> to Code2Git. <br> Resource not found. Make sure you enter the right repository name.`,
      );
      $('#error').show();
      break;
      ...
      if (xhr.status === 200) {
        ...
        if (!bool) {
          ...
          chrome.storage.local.set({ mode_type: 'hook' }, () => {
            console.log(`Error linking ${name} to Code2Git`);
          });
          /* Set Repo Hook to NONE */
          const hookKey = $('#platform').val() === 'gfg' ? 'gfg_hook' : 'code2git_hook';
          chrome.storage.local.set({ [hookKey]: null }, () => {
          ...
          chrome.storage.local.set({ mode_type: 'commit', repo: res.html_url }, () => {
            $('#error').hide();
            $('#success').html(
              `Successfully linked <a target="blank" href="${res.html_url}">${name}</a> to Code2Git. Start coding now!`,
            );
            $('#success').show();
            $('#unlink').show();
          });
          /* Set Repo Hook */
          const hookKey = $('#platform').val() === 'gfg' ? 'gfg_hook' : 'code2git_hook';
          const statsKey = $('#platform').val() === 'gfg' ? 'gfg_stats' : 'stats';
          chrome.storage.local
            .set({ [hookKey]: res.full_name })
            .then(() => {
              console.log('Successfully set new repo hook');
...
const unlinkRepo = () => {
  const hookKey = $('#platform').val() === 'gfg' ? 'gfg_hook' : 'code2git_hook';
  if (!$('#platform').val()) {
    alert("Please select a platform first to unlink.");
    return;
  }
  /* Set Repo Hook to NONE */
  chrome.storage.local.set({ [hookKey]: null }, () => {
...
$('#hook_button').on('click', () => {
  ...
  if (!$('#platform').val()) {
    $('#error').text('No platform selected - Pick LeetCode or GeeksForGeeks!');
    $('#error').show();
  } else if (!option()) {
    $('#error').text(
    ...
    */
    chrome.storage.local.get('code2git_token', data => {
      ...
      if (token === null || token === undefined) {
        /* Not authorized yet. */
        $('#error').text(
          'Authorization error - Grant Code2Git access to your GitHub account to continue (launch extension to proceed)',
        );
        $('#error').show();
        $('#success').hide();
      } else if (option() === 'new') {
        ...
        chrome.storage.local.get('code2git_username', data2 => {
          const username = data2.code2git_username;
          if (!username) {
            /* Improper authorization. */
            $('#error').text(
              'Improper Authorization error - Grant Code2Git access to your GitHub account to continue (launch extension to proceed)',
            );
            $('#error').show();
            $('#success').hide();
          } else {
...
$('#sync_counts').on('click', async () => {
  ...
  if (!platform) {
    $('#error').text('Please select a platform first!');
    $('#error').show();
    return;
  }
  const hookKey = platform === 'gfg' ? 'gfg_hook' : 'code2git_hook';
  const statsKey = platform === 'gfg' ? 'gfg_stats' : 'stats';

  //Check if linked to repo
  chrome.storage.local.get(hookKey, data => {
  ...
  const repo = await chrome.storage.local.get(hookKey).then(data => {
    const hook = data[hookKey];
    if (hook == null) {
      $('#error').text('No repository linked - Please link a repository to sync counts!');
      $('#error').show();
  ...
  const token = await chrome.storage.local.get('code2git_token').then(({ code2git_token }) => {
    if (code2git_token == null) {
      $('#error').text('No token found - Please authorize Code2Git to access your GitHub account!');
      $('#error').show();
      return;
    } else {
      $('#error').hide();
    }
    return code2git_token;
  });
